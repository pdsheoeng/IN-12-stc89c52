...
u8 gDS1302_TIME[7] = {0x00, 0x19, 0x10, 0x20, 0x04, 0x05, 0x21};
//00000000  00000000  00000011  00100000  00000100  00000101  00100001
//0100表示40 0111表示7；											
//00100011 首位为0采用24小时，第三位和第四位表示小时的十位，即10表2，后面4位表示小时的个位0011即3 
/*******************************************************************************
...
void ds1302_write_byte(u8 addr,u8 dat)
{
	...
	{
		DS1302_IO=addr&0x01;	 //与运算
		addr>>=1;				 //addr = addr向右位移
		DS1302_CLK=1;			 //由低电平变成高电平时，读操作开始,这里需要注意的是，当CE由低变高时，SCLK需要处于低电平
		_nop_();
...
{
	u8 i=0;
	ds1302_write_byte(0x8E,0X00);	//关闭burst模式(一次性读出这七个寄存器的所有数据)
	for(i=0;i<7;i++)
	{
		ds1302_write_byte(gWRITE_RTC_ADDR[i],gDS1302_TIME[i]); //时间写入	
	}
	ds1302_write_byte(0x8E,0X80); //开起burst模式	
}
